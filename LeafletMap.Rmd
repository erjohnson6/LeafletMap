...
maxDate <- max(confirmed$Date)
lastConfirmed <- filter(confirmed, Date == maxDate)
```
...
circleData <- with(lastConfirmed, data.frame(name = Country.Region, Confirmed = Confirmed, lat = Lat, lng = Long))
head(circleData)
circleData %>% leaflet() %>% 
   addTiles() %>%
   addCircles(weight = 5, radius = sqrt(circleData$Confirmed)*200, 
                    popup = paste(circleData$name, "-", round(circleData$Confirmed,-3)/1e6, "M"))
```
